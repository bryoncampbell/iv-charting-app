  ...
  try {
    ...
    const profiles =
      (await supabaseAdmin.from("profiles").select(
        "user_id, email, role, is_active, updated_at, first_name, last_name, date_of_birth, phone, street_address, city, state, zip_code, license_type, license_number, license_state, license_expiry"
      )).data ?? [];
